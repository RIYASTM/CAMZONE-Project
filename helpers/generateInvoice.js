  ...
  const doc = new PDFDocument({ margin: 20 });
    ...
    order.orderedItems.forEach(item => {
      if(item.itemStatus !== 'Cancelled' && item.itemStatus !== 'Returned'){
        const rowY = doc.y;
        doc.text(item.product.productName || 'N/A', 50, rowY);
    ...
    doc.text(Number(order.shipping || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 }), 400, summaryStartY + 20, { align: 'right', width: 150 });
